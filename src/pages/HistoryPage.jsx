...
export const HistoryPage = () => {
  ...
  const filterOptions = [
    { value: 'all', label: 'All Scans', count: scanHistory.length },
    { value: 'diseased', label: 'Diseased', count: getFilteredScans('diseased').length },
    { value: 'healthy', label: 'Healthy', count: getFilteredScans('healthy').length }
  ];
      ...
      {searchedScans.length === 0 ? (
        <Card className="text-center py-12 animate-slide-up">
          ...
          <h2 className="text-2xl font-bold text-gray-700 mb-2">
            {scanHistory.length === 0 ? 'No scans yet' : 'No scans found'}
          </h2>
        ...
        <>
          ...
          <div className="mb-6 p-4 bg-leaf-50 rounded-lg border border-leaf-200 animate-slide-up">
            <p className="text-sm text-leaf-800">
              Showing <strong>{searchedScans.length}</strong> of <strong>{scanHistory.length}</strong> total scans
            </p>
          </div>
